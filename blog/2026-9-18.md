---
title: 在 EKS 上搭建 Metrics、Logs、Traces 一体化可观测平台
authors: [Jacob]
tags: [EKS, Kubernetes, 可观测性, Prometheus, OpenTelemetry, Loki, Tempo, Grafana, Nightingale]
description: 一套覆盖指标、日志与链路追踪的 EKS 生产可观测平台实践，包含整体架构、部署参数、尾部采样、Span Metrics、自监控、故障边界与高可用演进路线。
...
本文同时收录于[知识库：EKS Metrics、Logs、Traces 一体化可观测平台](/docs/容器/EKS一体化可观测平台)。
